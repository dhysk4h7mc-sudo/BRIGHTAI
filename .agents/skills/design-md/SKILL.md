---
name: design-md
description: Analyze existing designed screens, UI systems, and visual patterns, then synthesize them into a semantic `DESIGN.md` reference for reuse in future frontend work. Use when Codex needs to inspect a current design language, extract reusable tokens and patterns, document spacing and typography rules, or turn multiple screens into a coherent design system reference. Trigger on requests about design system extraction, documenting UI rules, تحليل التصميم الحالي, إنشاء DESIGN.md, توثيق نظام التصميم, أو تحويل الشاشات الحالية إلى مرجع تصميمي.
allowed-tools:
  - "stitch*:*"
  - "Read"
  - "Write"
...
Use this skill only when the source of truth is a Stitch project or a Stitch-derived screen export. For normal BrightAI frontend work without Stitch access, use `frontend-design` instead.
...
If Stitch MCP access is unavailable, stop and state that this skill is not applicable in the current environment rather than improvising a fake Stitch workflow.
