...
name: seo-sitemap
description: >
  Analyze, validate, generate, and repair XML sitemaps for websites, including
  sitemap structure, URL quality, inclusion rules, and search-engine
  compatibility. Use when the user needs a new sitemap, wants to debug sitemap
  errors, or wants to review sitemap coverage and format. Trigger on "sitemap",
  "generate sitemap", "sitemap issues", "XML sitemap", خريطة الموقع, ملف
  sitemap, إنشاء sitemap, أو مشاكل خريطة الموقع.
allowed-tools:
  - Read
  - Grep
  - Glob
...
For BrightAI tasks:
- Write sitemap findings, architecture notes, and recommendations in Arabic
- Keep XML tags, URLs, and file names in English
- Plan around BrightAI's real structure: homepage, services, sectors, docs, blog, tools, and bilingual page pairs where present
