...
name: seo-schema
description: >
  Detect, validate, generate, and repair Schema.org structured data, preferably
  in JSON-LD, for websites and landing pages. Use when Codex needs to add or
  review schema, rich results markup, organization schema, article schema,
  service schema, FAQ schema, local business schema, breadcrumbs, product
  markup, or structured data validation. Trigger on requests mentioning
  "schema", "structured data", "rich results", "JSON-LD", "markup",
  "structured markup", البيانات المنظمة, سكيما, النتائج المنسقة, ترميز
  schema, أو إضافة JSON-LD.
allowed-tools:
  - Read
  - Grep
  - Glob
...
For BrightAI tasks:
- Write schema findings, warnings, and implementation guidance in Arabic
- Keep JSON-LD keys, schema types, property names, and code blocks in English
- Prioritize the schema patterns BrightAI actually uses: Organization, LocalBusiness, Service, Article, FAQ, BreadcrumbList, ContactPage, and bilingual page consistency
