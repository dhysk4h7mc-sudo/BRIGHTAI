...
name: seo-local
description: >
  Analyze and improve local SEO for businesses, branches, and service-area
  brands, including Google Business Profile optimization, NAP consistency,
  citations, local schema, reviews, local landing pages, map pack visibility,
  and multi-location strategy. Use when the user asks about local rankings,
  branch visibility, maps results, Google Business Profile, or city-based SEO.
  Trigger on "local SEO", "Google Business Profile", "GBP", "map pack",
  "local pack", "citations", "NAP consistency", "local rankings", "service
  area", "multi-location", "local search", السيو المحلي, خرائط Google, ملف
  النشاط التجاري, نتائج الخرائط, الفروع, أو تحسين الظهور المحلي.
allowed-tools:
  - Read
  - Grep
  - Glob
...
For BrightAI tasks:
- Write all local SEO findings, NAP issues, GBP notes, and recommendations in Arabic
- Keep schema keys, directory names, URLs, and markup examples in English
- Treat BrightAI as a Saudi business with Riyadh-based LocalBusiness signals and nationwide enterprise intent, so distinguish between local trust signals and broad Saudi market targeting

## Current Data Rule

Google Business Profile features, local ranking studies, and AI-assisted local search behavior change frequently. Verify any numeric claim or platform-specific product change with current sources before quoting it in a final recommendation.
...
Primary category selection is one of the strongest local relevance signals. An obviously wrong primary category is a major negative factor.
...
- Q&A or FAQ-style trust content on the site when GBP surface changes reduce direct visibility
- Current verification or trust-badge signals if present in the market at review time
- GBP link URL strategy, verified against current local SEO guidance before recommending a landing-page target
...
Review recency and steady review acquisition often matter more than raw lifetime review count.

**Check for:**
- Total Google review count visible on page or schema (magic threshold: 10, Sterling Sky)
- Star rating and sentiment thresholds appropriate to the category
