...
name: seo-hreflang
description: >
  Audit, validate, generate, and repair hreflang implementations for
  multilingual and multi-region websites. Use when Codex needs to review
  language targeting, regional targeting, alternate language URLs, or
  international SEO setup. Trigger on "hreflang", "i18n SEO", "international
  SEO", "multi-language", "multi-region", "language tags", hreflang, تعدد
  اللغات, تعدد المناطق, السيو الدولي, أو استهداف اللغة والمنطقة.
allowed-tools:
  - Read
  - Grep
  - Glob
...
For BrightAI tasks:
- Write all validation notes, fixes, and implementation guidance in Arabic
- Keep hreflang values, canonical tags, URLs, and markup examples in English
- Treat BrightAI as using `ar-SA` primary pages with `en-SA` companion pages and validate parity, reciprocity, and `x-default` behavior accordingly
