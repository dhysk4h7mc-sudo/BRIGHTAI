...
name: seo-technical
description: >
  Run technical SEO audits across crawlability, indexability, robots rules,
  sitemaps, status codes, canonicalization, mobile behavior, JavaScript
  rendering, Core Web Vitals, security headers, and site architecture. Use when
  the user reports crawl problems, indexing issues, speed concerns, rendering
  issues, or other technical search problems. Trigger on "technical SEO",
  "crawl issues", "robots.txt", "Core Web Vitals", "site speed", "security
  headers", السيو التقني, مشاكل الأرشفة, مشاكل الزحف, سرعة الموقع, أو
  مشاكل الفهرسة التقنية.
allowed-tools:
  - Read
  - Grep
  - Glob
...
For BrightAI tasks:
- Write all technical findings, risks, and fixes in Arabic
- Keep header names, crawler tokens, schema keys, CWV metric names, and code snippets in English
- Treat BrightAI as a static-HTML-heavy site with embedded schema, bilingual pages, and Saudi trust/compliance messaging that must remain crawlable in raw HTML
