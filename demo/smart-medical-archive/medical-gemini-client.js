...
function H() {
  ...
  j.apiBase = e2 || null, j.serverGateway = "", j.geminiModel = n2 || E, F(r, j.apiBase || ""), F(v, ""), F(y, j.geminiModel);
}
function Q() {
  return false;
}
...
function buildGatewayDemoResponse(payload, cause) {
  ...
  if (action === "insights") {
    ...
    return { result: buildGatewayDemoInsights(records), model: E + " demo", demo: true, error: cause && cause.message ? cause.message : "demo fallback" };
  }
  return { result: buildGatewayDemoExtract(payload && payload.reportText), model: E + " demo", demo: true, error: cause && cause.message ? cause.message : "demo fallback" };
}
function buildGatewayDemoExtract(reportText) {
...
function buildGatewayDemoInsights(records) {
  ...
  return {
    ...
    recommendations: [{ priority: "قريبة", action: "فعّل مراجعة عينة البيانات واضبط تكامل Gateway قبل التشغيل الإنتاجي." }],
    topDiagnoses: top(diagnosisCounts),
    topMedications: top(medicationCounts)
  };
}
function buildGatewayDemoAgentResponse(payload, cause) {
  const records = J(payload && payload.records);
  return {
    result: {
      summary: "تم تشغيل وضع demo fallback لأن خدمة Gateway أو الخادم الموحد لم يستجب.",
      actions: ["راجع السجلات عالية الخطورة أولاً", "فعّل Backend موحد يحتوي مفاتيح Gateway قبل العرض الإنتاجي", "اختبر مسار رفع تقرير ثم استخراج ثم حفظ ثم بحث"],
...
async function ce(e2) {
  U();
  e2 = Object.assign({ expected_schema: "smart_medical_archive_v2", production_rule: "Gateway must be called from backend only; never request API keys from browser UI." }, e2 || {});
  ...
  if (window.BRIGHTAI_ALLOW_DIRECT_GEMINI === true) try {
    const t3 = await async function(e3) {
      var t4 = K();
      if (!t4.key) throw new Error("Backend Gateway غير متوفر. لا تُدخل مفاتيح Gateway في الواجهة.");
      ...
      try {
        ...
        if (e4 && "AbortError" === e4.name) throw new Error("صار خلل بسيط، لا تشيل هم. نعرض لك نتيجة توضيحية آمنة الحين وتقدر تعيد المحاولة.");
      ...
      if (!u2.ok) {
        ...
        if (401 === u2.status) throw new Error("صار خلل بسيط، لا تشيل هم. نعرض لك نتيجة توضيحية آمنة الحين وتقدر تعيد المحاولة.");
        if (429 === u2.status) throw new Error("صار خلل بسيط، لا تشيل هم. نعرض لك نتيجة توضيحية آمنة الحين وتقدر تعيد المحاولة.");
        if (413 === u2.status) throw new Error("النص طويل جداً لمعالجة Gateway. قلّل حجم التقرير وحاول مجدداً.");
        if (u2.status >= 500) throw new Error("صار خلل بسيط، لا تشيل هم. نعرض لك نتيجة توضيحية آمنة الحين وتقدر تعيد المحاولة.");
        throw new Error("صار خلل بسيط، لا تشيل هم. نعرض لك نتيجة توضيحية آمنة الحين وتقدر تعيد المحاولة.");
      }
      try {
        l2 = await u2.json();
      } catch (e4) {
        throw new Error("تعذر قراءة استجابة Gateway. حاول مرة أخرى.");
      }
      if (!(l2 && l2.choices && l2.choices.length && l2.choices[0].message)) throw new Error("استجابة Gateway فارغة أو غير صالحة.");
      var p2 = l2.choices[0].message.content || "", h2 = l2.model || t4.model;
      if ("extract" === n2) {
        var f2 = z(p2);
        if (!f2) throw new Error("فشل في تحليل JSON المُرجع من Gateway. حاول بنص أوضح.");
    ...
    return R.connectionStatus && Z(R.connectionStatus, "تم تنفيذ اتصال Gateway المباشر في وضع تطوير مصرح فقط.", "success"), t3;
  } catch (e3) {
    return buildGatewayDemoResponse(e2, e3 || r2);
  }
  return buildGatewayDemoResponse(e2, r2 || new Error("اتصال Gateway المباشر معطل في واجهة الإنتاج. استخدم Backend فقط."));
}
async function ue(e2, t2, r2) {
  ...
  if (e2 === "/api/ai/medical-agent") return buildGatewayDemoAgentResponse(t2, o2);
