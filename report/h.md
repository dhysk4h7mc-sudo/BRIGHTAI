  ...
  برومبت 1: نظام التصميم المُحسّن

  قم بتطوير kernel/assets/css/kernel.css بإضافة:
  1. تدرج حقيقي في Border Radius (sm:6px, md:10px, lg:16px, xl:24px)
  2. تأثير Aurora Background متحرك يتفاعل مع mouse position
  3. Micro-interactions على hover/focus لكل card و button
  4. نظام أنيميشن جديد: counter animation مع easing, card reveal staggered, page
   transitions
  5. Light mode كامل مع دعم كل المكونات
  6. أنماط هندسية إسلامية خفيفة كـ background pattern
  7. تحسين Glassmorphism مع fallback للمتصفحات القديمة
  8. إضافة design tokens جديدة: --shadow-aurora, --glow-brand, --gradient-brand
  حافظ على كل المتغيرات الحالية وأضف فوقها بدون تكسير أي صفحة.

  برومبت 2: صفحة Chat المُحسّنة
  
  قم بتطوير kernel/chat.html و kernel/assets/js/kernel-chat-client.js بإضافة:
  1. Markdown rendering في ردود AI مع دعم code blocks, tables, lists, bold,
  links
  2. SSE Streaming للردود بدل انتظار الرد كاملاً  - أظهر الكلمة بكلمة
  3. نظام Chat Sessions - المستخدم يمكنه إنشاء محادثات متعددة والتبديل بينها
  4. Smart Quick Examples تتغير حسب Compliance Pack المختار
  5. File Upload button يسمح برفع ملفات مع PII scan تلقائي قبل الإرسال
  6. Voice Input button مع Web Speech API للإدخال الصوتي العربي
  7. Thinking Process display - عرض 4 خطوات (PII Scan → Risk Assessment → Policy
   Check → Response) كـ animated steps
  8. Copy message, Regenerate response, Share to evidence buttons على كل رسالة
  9. أحسن الـ mobile experience مع bottom sheet للإعدادات والتاريخ
  أبقِ  كل الوظائف الحالية وأضف فوقها. استخدم marked.js أو ähnliche library للـ
  markdown.
  
  برومبت 3: Dashboard مع Real-time Updates

  قم بتطوير kernel/index.html بإضافة:
  1. Animated metric counters مع easing functions (ease-out-expo) بدل العد
  البسيط
  2. Risk Heat Map visualization تعرض توزيع المخاطر حسب القسم/النوع
  3. Timeline visualization للـ traces بدل جدول - كل trace كـ node في timeline
  4. System Health Card يعرض حالة كل provider, DB, API كـ unified dashboard
  5. Quick Actions section مع 6 إجراءات بدل 3 - أضف: إنشاء تقرير، اختبار
  سيناريو، مراجعة الامتثال
  6. Real-time notification bell مع آخر الـ alerts
  7. Keyboard shortcut overlay (اضغط ? لعرض Shortcuts)
  8. Better empty state مع illustration و CTA واضح عند عدم وجود traces
  حافظ على كل الـ API endpoints الحالية ونظام الـ polling. أضف WebSocket
  connection اختياري.

  برومبت 4: Command Palette + Global Search

  أضف ملف JS جديد kernel/assets/js/kernel-command-palette.js وحدث kernel-nav.js
  لإضافة:
  1. Command Palette (Ctrl+K / Cmd+K) overlay مع search field
  2. يبحث في: صفحات Kernel، أوامر (مسح المحادثة، تصدير تقرير)، إعدادات
  3. Navigation items: Dashboard, Chat, Approvals, Audit, Stats, Compliance,
  Policies, Connectors, Reports, Scenarios, Evidence
  4. Actions: Switch Provider, Toggle Theme, Clear Session, Export Data
  5. Recent items tracking في localStorage
  6. Keyboard navigation (↑↓ للتنقل، Enter للاختيار، Esc للإغلاق)
  7. Arabic RTL support كامل
  8. Smooth animations (fade in + scale)
  أضف الـ CSS في kernel.css و hook الـ JS في كل صفحات kernel عبر kernel-nav.js.

  برومبت 5: صفحة Approvals المُحسّنة

  قم بتطوير kernel/approvals.html بإضافة:
  1. Bulk Actions toolbar تظهر عند تحديد طلبات متعددة (approve all, reject all)
  2. SLA Timer على كل طلب - عداد تنازلي يتحول للون أصفر ثم أحمر
  3. Approval Comments modal - إضافة تعليق إلزامي عند الرفض واختياري عند القبول
  4. Risk Summary Card داخل كل طلب يعرض: PII detected, Risk score bar, Matched
  policies
  5. Advanced Filters: حسب التاريخ (range)، المخاطر (slider)، الحزمة (dropdown)،
   الحالة (tabs)
  6. Delegation button - تحويل الطلب لمراجع آخر
  7. Approve/Reject مع keyboard shortcuts (A لقبول، R لرفض)
  8. Sound notification عند وصول طلب جديد
  حافظ على كل الـ API endpoints و mock data. أضف real-time polling كل 15 ثانية.
