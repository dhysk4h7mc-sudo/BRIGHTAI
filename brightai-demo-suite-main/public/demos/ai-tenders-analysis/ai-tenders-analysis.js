/**
 * BrightAI — Demo: تحليل المناقصات بالذكاء الاصطناعي
 */
...
function getContext() {
  return {
    ...
    issuingEntity: document.getElementById('ctx-entity')?.value?.trim() || '',
    estimatedValue: document.getElementById('ctx-value')?.value?.trim() || '',
    companyExperience: document.getElementById('ctx-company-exp')?.value?.trim() || '',
  };
}
...
async function handleSubmit(e) {
  ...
  if (!text) { showToast('يرجى كتابة استفسارك أولاً.', 'warning'); input.focus(); return; }
