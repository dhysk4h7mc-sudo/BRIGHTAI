...
function getContext() {
  return {
    dataSource: document.getElementById('ctx-datasource')?.value?.trim() || '',
    period:     document.getElementById('ctx-period')?.value?.trim() || '',
    goal:       document.getElementById('ctx-goal')?.value?.trim() || '',
    industry:   document.getElementById('ctx-industry-data')?.value || '',
  };
}
...
async function handleSubmit(e) {
  ...
  if (!text) { showToast('يرجى كتابة سؤالك التحليلي أولاً.', 'warning'); return; }
...
function clearConversation() {
  ...
  showToast('تم مسح التحليل.', 'info', 2000);
}

function animateMetrics() {
  document.querySelectorAll('.metric-item__bar').forEach((bar) => {
