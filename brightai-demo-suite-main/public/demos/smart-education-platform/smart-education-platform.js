/**
 * BrightAI — Demo: منصة التعليم الذكي
 */
...
const DEMO_ID = 'smart-education-platform';
...
function getContext() {
  return {
    learnerLevel:  document.getElementById('ctx-learner-level')?.value || '',
    weeklyHours:   document.getElementById('ctx-weekly-hours')?.value?.trim() || '',
    learningStyle: document.getElementById('ctx-learning-style')?.value || '',
...
async function handleSubmit(e) {
  ...
  if (!text) { showToast('يرجى كتابة هدفك التعليمي أولاً.', 'warning'); input.focus(); return; }
