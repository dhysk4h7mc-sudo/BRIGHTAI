/**
 * BrightAI — Demo: الأتمتة الذكية للعمليات
 */
...
const DEMO_ID = 'smart-automation';
...
function getContext() {
  return {
    processName: document.getElementById('ctx-process')?.value?.trim() || '',
    frequency:   document.getElementById('ctx-frequency')?.value?.trim() || '',
    currentTime: document.getElementById('ctx-time')?.value?.trim() || '',
    systems:     document.getElementById('ctx-systems')?.value?.trim() || '',
  };
}
...
async function handleSubmit(e) {
  ...
  if (!text) { showToast('يرجى وصف العملية المراد أتمتتها.', 'warning'); input.focus(); return; }
