...
function se() {
  if (R.fileQueue) {
    ...
    R.fileQueue.innerHTML = j.uploadQueue.map(function(e2) {
      ...
      var n2;
      const r2 = J(e2.validation && e2.validation.issues).length, a2 = "queued" === e2.status ? "بانتظار" : "processing" === e2.status ? "قيد المعالجة" : "done" === e2.status ? "مكتمل" : "cancelled" === e2.status ? "ملغي" : "خطأ", o2 = e2.previewUrl ? "audio" === e2.kind ? `<div class="queue-preview"><audio controls preload="none" src="${_(e2.previewUrl)}"></audio></div>` : `<div class="queue-preview"><img src="${_(e2.previewUrl)}" alt="معاينة ${_(e2.name)}" /></div>` : "", i2 = ["processing" === e2.status ? `<button class="queue-action" data-action="cancel" data-id="${_(e2.id)}">إلغاء</button>` : "", "cancelled" === e2.status || "error" === e2.status ? `<button class="queue-action" data-action="resume" data-id="${_(e2.id)}">استئناف</button>` : "", e2.extractedText ? `<button class="queue-action" data-action="use" data-id="${_(e2.id)}">استخدام النص</button>` : "", `<button class="queue-action" data-action="remove" data-id="${_(e2.id)}">إزالة</button>`].filter(Boolean).join("");
      return `
